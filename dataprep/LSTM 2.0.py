...
train_data = mid_prices[:split] # 2324 DataPoints
...
# Normalisiere BEIDE Daten mit Respekt zu den TrainingsDaten, da ich keinen Zugang zu den Test-Daten habe! -> hier kann man auch nochmal überlegen den einfachen Scaler zu verwenden
...
# Normalisiere das letzte Teilstück 2453 - 2000
...
#print('MSE error for standard averaging: %.5f'%(0.5*np.mean(mse_errors))) # MSE: 0.00209
...
#print('MSE error for EMA averaging: %.5f'%(0.5*np.mean(mse_errors))) # MSE 0.00012
...
# Beide Beispiele verwenden nur die Vorhersage von 1 Tag in die Zukunft, für unsere Zwecke sind allerdings mehrere Tage in die Zukunft wichtig


###########################################


### LSTM Für Stock Movement Predictions ###

# Data Generator
# Habe ich nicht unbedingt verstanden was es damit auf sich hat
...
### Def. Hyperparameter
D = 1 # Dimension unserer Daten, wir haben nur eine Dimension, daher 1
num_unrollings = 5 # Wie viele Zeitschritte soll in die Zukunft geschaut werden?
batch_size = 10 # Wie viele Samples sollen in die batches?
num_nodes = [200,200,150] # Wie viele Hidden Nodes Pro Layer soll das LSTM haben?
n_layers = len(num_nodes) # Wie viele Layer? Ergibt sich aus num_nodes
dropout = 0.2 # Größe des Dropouts
...
### Def. Parameters des LSTM und Regression layer
lstm_cells = [
...
n_predict_once = 2 # Number of steps you continously predict for
...
test_points_seq = np.arange(2324,2583,10).tolist()
